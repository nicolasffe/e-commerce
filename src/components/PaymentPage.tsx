...
const methods = [
  ...
  { id: "paypal", label: "PayPal", desc: "Saldo ou cartão" },
] as const
        ...
        <div className="grid gap-10 lg:grid-cols-[1.05fr,0.95fr]">
          <div className="space-y-6">
            <div>
              ...
              <h1 className="text-3xl font-semibold text-slate-900">Pagamento</h1>
              <div className="mt-3 flex gap-4 text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">
                <span>Pedido</span>
                <span>Informações</span>
                <span className="text-slate-900">Pagamento</span>
              </div>
            </div>

            <div className="rounded-3xl bg-white/80 p-6 shadow-sm ring-1 ring-amber-100 backdrop-blur">
              <p className="text-sm font-semibold text-slate-900">Método</p>
              <div className="mt-4 grid gap-3 sm:grid-cols-3">
                {methods.map((item) => (
                  <button
                    key={item.id}
                    onClick={() => setMethod(item.id)}
                    className={`flex flex-col rounded-2xl border px-3 py-3 text-left text-sm font-semibold transition ${
                      method === item.id
              ...
              {method === "card" ? (
                <div className="mt-6 space-y-3">
                  <div className="grid gap-3 sm:grid-cols-2">
                    <input className={inputStyle} placeholder="Nome no cartão" />
                    <input className={inputStyle} placeholder="Número do cartão" />
                  </div>
                  <div className="grid gap-3 sm:grid-cols-3">
                    <input className={inputStyle} placeholder="Validade (MM/AA)" />
                    <input className={inputStyle} placeholder="CVV" />
                    <input className={inputStyle} placeholder="CPF/CNPJ do titular" />
                  </div>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <input className={inputStyle} placeholder="Parcelas" />
                    <input className={inputStyle} placeholder="Cupom de desconto" />
                  </div>
                </div>
              ) : null}

              {method === "pix" ? (
                <div className="mt-6 space-y-3 rounded-2xl bg-amber-50 p-4 ring-1 ring-amber-100">
                  <p className="text-sm font-semibold text-slate-900">
                    Pague com Pix e libere o ingresso na hora
                  </p>
                  <p className="text-sm text-slate-700">
                    Ao confirmar, exibiremos o QR Code Pix e o código copia e cola. Validade de 15
                    minutos.
                  </p>
                </div>
              ) : null}
              ...
              {method === "paypal" ? (
                <div className="mt-6 space-y-3 rounded-2xl bg-slate-50 p-4 ring-1 ring-amber-100">
                  <p className="text-sm font-semibold text-slate-900">Pagar com PayPal</p>
                  <p className="text-sm text-slate-700">
                    Você será redirecionado para concluir com sua conta PayPal e voltar para a
              ...
              <div className="mt-6 flex items-center justify-between rounded-2xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white shadow-lg">
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-slate-200">Cartão, Pix e PayPal seguros</span>
                </div>
                <button
          ...
          <div className="rounded-3xl bg-white/90 p-6 shadow-lg ring-1 ring-amber-100 backdrop-blur">
            ...
            <div className="mt-6 space-y-2 rounded-2xl bg-amber-50 p-4 text-sm text-slate-800 ring-1 ring-amber-100">
              ...
              <p>Assim que o pagamento for confirmado, seu QR Code fica pronto para check-in.</p>
            </div>
          </div>
        </div>
      </div>
